    ...
    bcrypt
        ...
        .catch((e) => {
            if (e.code === 11000) {
                ...
                next(
                    new BadRequestError(
                        'Переданы неверные данные при создании пользователя',
                    ),
                );
            } else {
                next(e);
...
const editUserInfo = (req, res, next) => {
    ...
    User.findByIdAndUpdate(
        ...
        .orFail(() => {
            throw new NotFoundError('Пользователь с заданным _id не найден');
        })
        .then((user) => {
            res.send(user);
        })
        .catch((e) => {
            if (e.code === 11000) {
                ...
                next(
                    new BadRequestError(
                        'Переданы неверные данные при обновлении профиля',
                    ),
                );
            } else {
                next(e);
